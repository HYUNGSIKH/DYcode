...
!      global
      if(depth(k) .LE. 40.)frict=0.025
      if(depth(k) .gt. 40. .and. depth(k) .le. 60.)frict=0.020d0
     &-((depth(k)-40)*(0.0002d0/20.)) !frict=0.020
!    if(depth(k) .gt. 60. .and. depth(k) .le. 200.)frict=0.015d0-((depth(k)-60)*(0.0002d0/140.))!frict=0.015  
      if(depth(k) .gt. 60.)frict=0.023 
